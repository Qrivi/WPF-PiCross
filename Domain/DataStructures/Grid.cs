...
namespace DataStructures
{
    ...
    ///     If you need to be able to modify items in a grid, use a <see cref="IVar" /> subtype as type parameter, e.g.
    ///     <code>IGrid&lt;Cel&lt;T&gt;&gt;</code>.
    /// </summary>
    ...
    public static class Grid
    {
        ...
        public static IGrid<T> FromRows<T>(ISequence<ISequence<T>> rows)
        {
            ...
            return Create(size, p => rows[p.Y][p.X]);
        }
    }
    ...
    internal abstract class GridBase<T> : IGrid<T>
    {
        ...
        public IEnumerable<Vector2D> AllPositions
        {
            get
            {
                return from y in Enumerable.Range(0, Size.Height)
        ...
        {
            get { return Enumerable.Range(0, Size.Height); }
        }

        public IEnumerable<int> ColumnIndices
        {
            get { return Enumerable.Range(0, Size.Width); }
        }
        ...
        {
            get { return RowIndices.Select(Row); }
        }

        public IEnumerable<ISequence<T>> Columns
        {
            get { return ColumnIndices.Select(Column); }
        }
    ...
    internal class Grid<T> : GridBase<T>
    {
        ...
        {
            get { return new Size(items.GetLength(0), items.GetLength(1)); }
        }

        public override T this[Vector2D position]
        {
            get { return items[position.X, position.Y]; }
        }
    }
